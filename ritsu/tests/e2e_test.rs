...
"#,
    ...
    async fn start_server(&mut self) -> Result<()> {
        ...
        if !binary_path.exists() {
            anyhow::bail!(
                "ritsu-server binary not found at: {}",
                binary_path.display()
            );
        }
    ...
    async fn start_client_daemon(&mut self) -> Result<()> {
        ...
        let mut cmd = Command::new(&binary_path);
        cmd.arg("start").env("RUST_LOG", "info,ritsu=debug");
...
async fn test_server_startup_and_ping() -> Result<()> {
    ...
    assert!(
        success || output.contains("Running"),
...
async fn test_send_message_and_receive_response() -> Result<()> {
    ...
    assert!(
        success
            ...
            || output.contains(&format!("{}", nerd_font::categories::Fa::Android)),
...
async fn test_session_persistence() -> Result<()> {
    ...
    assert!(success1 || success2, "At least one message should succeed");
...
async fn test_task_management() -> Result<()> {
    ...
    // Create a task
    let (success, _) = run_command(&["task", "add", "Test task", "--priority", "high"])?;
...
async fn test_notes_and_memory() -> Result<()> {
    ...
    assert!(
        mem_success || mem_output.contains("memory"),
        "Should query memory"
    );
...
async fn test_concurrent_messages() -> Result<()> {
    ...
    let handles: Vec<_> = (0..3)
        .map(|i| {
            ...
            tokio::task::spawn_blocking(move || run_command(&["send", &msg]))
        })
        .collect();
...
async fn test_full_conversation_flow() -> Result<()> {
    ...
    let messages = vec!["Hello!", "Can you help me?", "Thank you!"];
