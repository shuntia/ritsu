...
pub async fn stop() -> Result<()> {
    ...
    match client
        .send_request(ritsu_common::protocol::ClientRequest::Shutdown)
        .await
    {
        Ok(_) => {
            println!(
...
async fn halt_server(noconfirm: bool) -> Result<()> {
    if !noconfirm {
        println!(
            "{}  WARNING: This will forcefully terminate the ritsu server.",
    ...
    if killed_count > 0 {
        ...
        println!(
            "{} Halted {} ritsu-server process(es)",
            nerd_font::categories::Fa::Check,
            killed_count
        );
    }
...
async fn halt_client(noconfirm: bool) -> Result<()> {
    if !noconfirm {
        println!(
            "{}  WARNING: This will forcefully close all ritsu GUI windows.",
            nerd_font::categories::Fa::ExclamationCircle
        );
        print!("Are you sure you want to continue? [y/N]: ");
    ...
    if killed_count > 0 {
        tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
        println!(
            "{} Halted {} ritsu client process(es)",
            nerd_font::categories::Fa::Check,
            killed_count
        );
    }
