...
pub async fn export_conversation(
    ...
    let session_to_export = if let Some(sid) = session_id {
        ...
        let response = client
            .send_request(ClientRequest::ListSessions { limit: Some(1) })
            .await?;
    ...
    let client = crate::ipc::IpcClient::new(super::get_client_socket()?);
    let response = client
        .send_request(ClientRequest::GetConversationHistory {
    ...
    if turns.is_empty() {
        println!(
            "No conversation history found for session {}",
            session_to_export
        );
        return Ok(());
    }
...
fn export_txt(
    file: &mut File,
    session_id: &str,
    turns: &[ritsu_common::protocol::ConversationTurn],
) -> Result<()> {
