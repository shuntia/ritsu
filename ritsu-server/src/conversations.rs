...
impl ConversationManager {
    ...
    pub async fn get_or_create_session(&self, session_id: &str) -> Result<ConversationSession> {
        ...
        self.db
            .call(move |conn| -> rusqlite::Result<ConversationSession> {
    ...
    pub async fn get_active_sessions(&self) -> Result<Vec<ConversationSession>> {
        self.db
            .call(|conn| -> rusqlite::Result<Vec<ConversationSession>> {
                let mut stmt = conn.prepare(
                    "SELECT session_id, turn_count, started_at, last_activity, title
                 FROM conversations
...
mod tests {
    ...
    async fn test_add_turn() {
        ...
        let turn_number = manager
            .add_turn("test_session", "user", "Hello", None, None, None)
            .await
            .unwrap();
    ...
    async fn test_get_history() {
        ...
        manager
            ...
            .unwrap();
        manager
            .add_turn("test_session", "assistant", "Hi there!", None, None, None)
            .await
            .unwrap();
